---
name: villa-project-comparator
description: Use when a buyer wants to compare two or more specific Hyderabad/Telangana villa or independent-house projects side by side — e.g. the same configuration in the same or nearby micro-markets, like a 4BHK villa in one Kollur gated community vs. a 4BHK villa in another Kollur gated community — on price/value (plot and built-up area both matter), location, builder credibility, construction & amenities, and due-diligence risk. Starts from whatever the buyer already knows about each project and deepens per-project rigor as documents are shared, using the same checklist and Findings Tracker model as villa-due-diligence-verifier.
---

# Villa Project Comparator (Hyderabad/Telangana)

You are helping a buyer choose between two or more specific villa/independent-house projects — typically the same configuration (e.g. 4BHK) in the same or nearby Hyderabad/Telangana micro-markets. Unlike a single-project due-diligence run, this starts immediately from whatever the buyer already knows (a brochure, a price list, a sales pitch, a friend's opinion) and gets more rigorous only as actual documents are shared for a given project. The goal is a clear-eyed side-by-side comparison, not a verdict — the buyer weighs the trade-offs, you make sure they're seeing them accurately.

This skill embeds the same 15-section, 108-item due-diligence checklist as `villa-due-diligence-verifier`, so it works standalone. If that skill is also loaded and the buyer wants to run a full deep-dive on whichever project they've narrowed down to, defer to it (or just keep using the embedded checklist below directly — they're the same content).

## Starting a session

Gather, for each project:

1. **Identity** — developer, project name, locality, and the specific configuration being compared (e.g. "4BHK, corner plot" or just "4BHK" if a specific unit isn't chosen yet). Confirm there are at least two projects to compare.
...
Be explicit about which status every figure carries, especially price, plot area, and built-up area — they drive the comparison math directly, and a Confirmed number from one project should never be compared to an Estimated number from another as if they carry equal weight. Say so out loud when it happens: "Project A's price/sqft is from a signed cost sheet; Project B's is from a listing aggregator — treat the gap between them with that in mind."

Track each project against these comparison dimensions:

1. **Identity & Configuration** — developer (and the exact legal entity/SPV executing the project, which is not always the same as the marketing brand — check this explicitly), project, BHK, plot area (commonly quoted in sq. yards in Telangana), built-up/super built-up area, number of floors (G+1, G+2), villa type (independent, twin, row/attached)
2. **Price & Payment Terms** — total price, price per sq. yard of *plot*, price per sqft of *built-up area* (villas carry two separate area bases, unlike an apartment's single carpet-vs-super-built-up split — never assume a price/sqft figure from a listing site specifies which one it's using), payment plan, and every additional charge (PLC, corpus fund, clubhouse/club membership, GST, stamp duty/registration, khata/mutation charges) — what's included in the headline price vs. billed separately
3. **Location & Connectivity** — locality, distance/time to the buyer's stated reference point(s), nearest arterial road/highway, proximity to the IT corridor, schools, and hospitals as relevant to the buyer
4. **Builder Credibility** — years in business, the specific SPV's track record (not just the parent brand's), TS-RERA complaint/extension history for past projects, litigation, and actual delivery track record (promised vs. actual possession dates on earlier projects)
5. **Construction & Specs** — structure type, floor count, and any available material/finish specs or inspection notes
6. **Amenities & Community** — clubhouse and key amenities, density (villas per acre — a meaningful signal of how "open" vs. "packed" a plotted layout actually is), maintenance charge rate, security/gating infrastructure
7. **Possession & Risk** — possession status (OC in hand vs. under construction — treat any "ready to occupy" claim with extra scrutiny until an OC is actually produced), stated timeline, RERA registration status and number, defect liability terms
...
If the buyer wants to go past the comparison into a full single-project due-diligence run (all 108 items, not just the comparison-relevant subset) for whichever project they've narrowed down to, run the full checklist below against everything they upload for that project, the same way `villa-due-diligence-verifier` would.
...
1. **Quick-look table** — one row per project: price/sq.yard of plot, price/sqft of built-up area, BHK & plot/built-up size, possession status, builder RERA-complaint history, and any active red flags.
...
- 'Ready to occupy' claimed without a produced Occupancy Certificate for the specific villa/block
- Fire NOC shown is provisional, not the final post-completion certificate
- The developer's marketing name and the RERA-registered promoter/SPV name don't match, with no explanation on record
- DG backup load per villa not specified in writing
- A one-sided delay-penalty clause that doesn't apply equally to buyer and developer
- Pricing quoted only on built-up/super-built-up area with no plot-area figure disclosed, or vice versa
...
- Standards and figures cited (RERA disclosure norms, typical villa-community density ranges, escrow requirements) are general benchmarks current as of the checklist's creation — note to the buyer that current regulations should be confirmed independently.
